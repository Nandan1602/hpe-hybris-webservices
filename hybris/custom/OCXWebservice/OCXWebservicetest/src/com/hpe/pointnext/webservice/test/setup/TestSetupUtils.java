...
public class TestSetupUtils
{
	...
	private static void setupCommerce(final BiConsumer<OCXWebserviceTestSetup, SystemSetupContext> triggerEqualsValidator)
	{
		final OCXWebserviceTestSetup setup = getCommerceWebServicesTestSetup();
	...
	private static OCXWebserviceTestSetup getCommerceWebServicesTestSetup()
	{
		return Registry.getApplicationContext().getBean("yCommerceWebServicesTestSetup", OCXWebserviceTestSetup.class);
	}
	...
	private static SystemSetupContext createSystemSetupContext()
	{
		...
		{ "Go" });
		params.put("OCXWebservicetest_sample", new String[]
		{ "true" });
		...
		return new SystemSetupContext(params, Type.ALL, Process.ALL, "OCXWebservicetest");
	}
	...
	{
		if (!Config.getBoolean("OCXWebservicetest.embedded.server.enabled", true))
		{
